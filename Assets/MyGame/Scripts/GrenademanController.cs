...
public class GrenademanController : ExRbStateMachine<GrenademanController>
{
    ...
    class Appearance : ExRbState<GrenademanController, Appearance>
    {
        enum SubStateId
        {
            Float,
        ...
        protected override void Enter(GrenademanController ctr, int preId, int subId)
        {
            ctr._exRb.BoxCollider.enabled = false;
            this.TransitSubReady((int)SubStateId.Float);
        }

        protected override void FixedUpdate(GrenademanController ctr)
        {
            ctr._gravity.UpdateVelocity();
            ctr._exRb.velocity = ctr._gravity.CurrentVelocity;
        }

        protected override void OnBottomHitStay(GrenademanController ctr, RaycastHit2D hit)
        {
            ctr._gravity.Reset();
        }

        class Float : ExRbSubState<GrenademanController, Float, Appearance>
        {
            protected override void Enter(GrenademanController ctr, Appearance parent, int preId, int subId)
            {
                ctr._animator.Play(AnimationNameHash.Float);
            }

            protected override void OnBottomHitEnter(GrenademanController ctr, Appearance parent, RaycastHit2D hit)
            {
                parent.TransitSubReady((int)SubStateId.Pause);
            }

            protected override void OnTriggerEnter(GrenademanController ctr, Appearance parent, Collider2D collision)
            {
                ctr._exRb.BoxCollider.enabled = true;
            }
        }

        class Pause : ExRbSubState<GrenademanController, Pause, Appearance>
        {
            protected override void Enter(GrenademanController ctr, Appearance parent, int preId, int subId)
            {
            ...
            protected override void Update(GrenademanController ctr, Appearance parent)
            {
            ...
            protected override void Exit(GrenademanController ctr, Appearance parent, int nextId)
            {
                // 通知を飛ばす
        ...
        class Wait : ExRbSubState<GrenademanController, Wait, Appearance> { }
    }

    class Idle : ExRbState<GrenademanController, Idle>
    {
        protected override void Enter(GrenademanController ctr, int preId, int subId)
        {
        ...
        protected override void Update(GrenademanController ctr)
        {
        ...
        protected override void OnTriggerEnter(GrenademanController ctr, RockBusterDamage collision)
        {
            ctr.boss.Damaged(collision);
        }
    }

    class Jumping : ExRbState<GrenademanController, Jumping>
    {
        float jump_vel =30f;
        float vel_x;
    ...
    class PlaceBomb : ExRbState<GrenademanController, PlaceBomb>
    {
        float jump_vel = 30f;
        float vel_x;
        int layerMask = LayerMask.GetMask("Ground");
    ...
    class Run : ExRbState<GrenademanController, Run>
    {
        public Run()
        {
        ...
        class Start : ExRbSubState<GrenademanController, Start, Run>
        {
            int animationHash = Animator.StringToHash("RunStart");

            protected override void Enter(GrenademanController ctr, Run parent, int preId, int subId)
            {
                ctr.boss.TurnToTarget(WorldManager.Instance.PlayerController.transform.position);
                ctr._animator.Play(animationHash);
            }

            protected override void Update(GrenademanController ctr, Run parent)
            {
        ...
        class Running : ExRbSubState<GrenademanController, Running, Run>
        {
            ...
            protected override void FixedUpdate(GrenademanController ctr, Run parent)
            {
            ...
            protected override void OnLeftHitStay(GrenademanController ctr, Run parent, RaycastHit2D hit)
            {
                ctr.TransitReady((int)StateId.Idle);
            }
            protected override void OnRightHitStay(GrenademanController ctr, Run parent, RaycastHit2D hit)
            {
                ctr.TransitReady((int)StateId.Idle);
            }
        }
    ...
    class Shoot : ExRbState<GrenademanController, Shoot>
    {
        public Shoot() {
        ...
        class Hold: ExRbSubState<GrenademanController, Hold, Shoot>
        {
            protected override void Enter(GrenademanController ctr, Shoot parent, int preId, int subId)
            {
                if (preId != 2) ctr._timer.Start(0.4f, 0.6f);
                else ctr._timer.Start(0, 0);
            }

            protected override void Update(GrenademanController ctr, Shoot parent)
            {
                ctr._timer.MoveAheadTime(Time.deltaTime, () =>
                {
                    parent.TransitSubReady(1);
        ...
        class Fire: ExRbSubState<GrenademanController, Fire, Shoot>
        {
            protected override void Enter(GrenademanController ctr, Shoot parent, int preId, int subId)
            {
            ...
            protected override void Update(GrenademanController octrbj, Shoot parent)
            {
                parent.TransitSubReady(2);
            }
        }

        class Stiffness : ExRbSubState<GrenademanController, Stiffness, Shoot>
        {
            protected override void Enter(GrenademanController ctr, Shoot parent, int preId, int subId)
            {
